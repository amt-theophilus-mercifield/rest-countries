  ...
  return (
    ...
    <Link to= '/'><StyledBtn className="styledBtn px-8 py-2 mt-8 mx-4 sm:mx-8 md:mx-12 lg:mx-16 rounded shadow-md">&larr; Back</StyledBtn></Link>
      <Section className="w-full flex justify-between items-center px-4 sm:px-8 md:px-12 lg:px-16 my-20">
        {country.map((item) => (
          <Wrap className="flex flex-col lg:flex-row gap-8 lg:justify-center lg:items-center" key={item.cca3}>
            ...
            <Article className="lg:basis-1/2">
              ...
              <ul className="my-4 flex flex-col items-start justify-start gap-2">
                <li>Native name: {item.name.nativeName?.eng?.common || item.name.official}</li>
